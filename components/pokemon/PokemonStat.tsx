...
export function PokemonStat({ style, color, name, value, ...rest }: Props) {
  ...
  const baInnerStyle = useAnimatedStyle(()=> {
    return {
        flex: sharedValue.value
    }
  })
  const baBackgroundStyle = useAnimatedStyle(()=> {
    return {
  ...
  return (
    <Row gap={8} style={[style, styles.root]} {...rest}>
      ...
      <View style={styles.number}>
        <ThemedText>{value.toString()}</ThemedText>
      </View>
      <Row style={styles.bar}>
        <Animated.View
          style={[styles.barInner, { backgroundColor: color }, baInnerStyle]}
        />
        <Animated.View
          style={[styles.barBackground, {backgroundColor: color }, baBackgroundStyle]}
        />
      </Row>
    </Row>
  );
